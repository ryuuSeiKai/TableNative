...
<Steps>
  <Step title="Debounced Auto-Save">
    Tab state auto-saves 500ms after any change (debounced to avoid rapid successive writes)
  </Step>
  ...
  <Step title="Restoration on Connect">
    When you reconnect to a database, TablePro restores saved tabs and re-executes queries for table tabs
  </Step>
  <Step title="Window Close Handling">
    When you close a window, any pending debounced save flushes immediately to prevent data loss
  </Step>
</Steps>
...
Pin tabs with queries you refer to frequently. This prevents them from being accidentally closed.
...
While tabs are lightweight, closing tabs you no longer need keeps the tab bar clean and makes navigation easier.
